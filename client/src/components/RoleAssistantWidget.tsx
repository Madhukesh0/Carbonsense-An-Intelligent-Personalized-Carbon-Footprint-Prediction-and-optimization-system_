...
export default function RoleAssistantWidget({ role }: { role: keyof typeof rolePresentation }) {
  ...
  return <><button type="button" onClick={() => setOpen((value) => !value)} aria-label={open ? "Close CarbonSense assistant" : `Open ${presentation.label}`} className="fixed bottom-6 right-6 z-50 grid h-14 w-14 place-items-center rounded-full bg-gradient-to-br from-emerald-500 to-emerald-800 text-xl text-white shadow-xl shadow-primary/20 transition-transform hover:scale-105 active:scale-[.97]" title={presentation.label}>{open ? <X size={23} /> : presentation.icon}</button>{open && <section className="fixed bottom-24 right-4 z-50 w-[min(390px,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-border bg-white shadow-2xl dark:border-white/10 dark:bg-[#13261f]"><header className="flex items-center gap-3 bg-gradient-to-r from-emerald-700 to-emerald-800 px-5 py-4 text-white"><span className="grid h-9 w-9 place-items-center rounded-xl bg-white/15 text-lg">{presentation.icon}</span><span className="min-w-0 flex-1"><b className="block text-sm">{presentation.label}</b><small className="block text-[11px] text-primary">Grounded CarbonSense guidance</small></span><Bot size={18} /></header><AIChatBox className="border-0 shadow-none" height="420px" messages={messages} onSendMessage={send} isLoading={chat.isPending} placeholder="Ask about your CarbonSense workspace…" emptyStateMessage="Ask a question about your CarbonSense records." suggestedPrompts={[presentation.prompt, "Explain the model and data boundary."]} /><p className="border-t border-border px-4 py-2 text-[10px] leading-4 text-muted-foreground dark:border-white/10">Powered by Google Gemini, grounded in your CarbonSense workspace context. Planning guidance only — not verified emissions reductions.</p></section>}</>;
}
