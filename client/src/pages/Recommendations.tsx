...
export default function Recommendations() {
  ...
  return <RepoShell title="Recommendations"><main className="cs-page"><Card className="cs-card mx-auto max-w-4xl p-6 sm:p-8"><p className="cs-kicker text-primary">Completed-result actions</p><h1 className="mt-3 text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">Recommendations</h1><p className="mt-3 text-base leading-relaxed text-muted-foreground">Profile-matched actions are selected from your completed AI prediction and transparent baseline. They are planning guidance, not verified emissions reductions.</p>{!data?.available ? <div className="mt-6 rounded-2xl bg-amber-50 p-5 text-sm text-amber-900"><p className="font-semibold">Complete your result first</p><p className="mt-2">{data?.reason || "Loading your completed result…"}</p></div> : <><div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-[#f6f8f7] p-4"><p className="text-xs font-semibold uppercase tracking-[.12em] text-muted-foreground">Starting AI result</p><p className="mt-2 font-mono text-2xl">{data.predictionKg} kg</p></div><div className="rounded-xl bg-primary/8 p-4"><p className="text-xs font-semibold uppercase tracking-[.12em] text-primary">Starting baseline</p><p className="mt-2 font-mono text-2xl text-primary">{data.baselineKg} kg</p></div></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{data.recommendations.map(item => <div key={item.key} className="rounded-2xl bg-[#f6f8f7] p-5"><div className="flex justify-between gap-3"><h2 className="font-semibold">{item.title}</h2><span className="text-sm font-semibold text-[#157f54]">−{item.estimatedReductionKg} kg</span></div><p className="mt-2 text-base leading-relaxed text-muted-foreground">{item.description}</p><p className="mt-3 text-xs font-medium text-[#157f54]">{item.provenance}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Why shown: {item.trigger}</p><Button size="sm" className="mt-4 bg-[#157f54] hover:bg-[#106b47]" onClick={() => accept.mutate({ recommendation_key: item.key })} disabled={accept.isPending}>Accept recommendation</Button></div>)}</div></>}{(resultLed.error || accept.error) && <p role="alert" className="mt-5 rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-base text-destructive">{(resultLed.error || accept.error)?.message}</p>}</Card></main></RepoShell>;
}
