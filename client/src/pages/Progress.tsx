    ...
    <RepoShell>
      <main className="cs-page cs-progress-page">
        ...
        <section className="cs-progress-heading mt-5">
          <h1 className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">Your Progress</h1>
        ...
        <section className="cs-progress-overview cs-card mt-8" aria-labelledby="progress-overview-title">
          ...
          <div><h2 id="progress-overview-title" className="text-xl font-bold tracking-[-0.04em] text-foreground">Level up your sustainability</h2><p className="mt-1 text-base leading-relaxed text-muted-foreground">{canViewContributors ? "Earn recognition from recorded participation, maintain your engagement streak, and explore consent-filtered aggregate patterns." : "Earn recognition from recorded participation and maintain your engagement streak."}</p></div>
        </section>
        ...
        {showQuests ? (
          <section id="progress-quests-panel" role="tabpanel" className="mt-8">
            <section className="cs-progress-celebrate cs-card" aria-labelledby="progress-intro-title">
              ...
              <div><p className="cs-data-label">CarbonQuest</p><h2 id="progress-intro-title" className="mt-1 text-xl font-bold tracking-[-0.04em] text-foreground">Make consistent climate participation visible.</h2><p className="mt-2 text-base leading-relaxed text-muted-foreground">Recognition reflects recorded participation and administrator-verified action completion. It does not prove real-world emissions reductions.</p></div>
            </section>
            <div className="mt-5 grid gap-5 md:grid-cols-3">
              <article className="cs-progress-stat cs-card"><ScoreGauge score={summary?.score ?? 0} maximum={1000} label="recorded points" /><p className="cs-data-label mt-5">CarbonQuest score</p><p className="mt-2 text-5xl font-bold tracking-[-0.07em] text-foreground">{summary?.score ?? 0}</p><p className="mt-2 text-base leading-relaxed text-muted-foreground">Recorded participation points</p></article>
              <article className="cs-progress-stat cs-card"><span className="cs-progress-stat-icon cs-progress-stat-icon--amber"><Flame size={24} /></span><p className="cs-data-label mt-5">Day streak</p><p className="mt-2 text-5xl font-bold tracking-[-0.07em] text-foreground">{summary?.streak ?? 0}</p><div className="mt-4"><StreakCalendar activeDays={Array.from({ length: Math.min(28, summary?.streak ?? 0) }, (_, index) => index + 1)} /></div><p className="mt-3 text-base leading-relaxed text-muted-foreground">Recorded engagement points in sequence</p></article>
              ...
              <article className="cs-card mt-5 p-6 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="cs-data-label">Badges</p><h2 className="mt-2 text-2xl font-bold tracking-tight text-foreground">Recognition from real records.</h2></div><Sparkles className="text-primary" size={23} aria-hidden="true" /></div>{summary?.badges?.length ? <div className="mt-6 grid gap-3 sm:grid-cols-2">{summary.badges.map((badge: string) => <BadgeCard key={badge} label={badge} detail="Recognition is based on recorded participation or administrator-verified action completion." />)}</div> : <div className="cs-progress-empty mt-6"><Award size={21} aria-hidden="true" /><div><p className="font-bold text-foreground">Your first recognition is waiting.</p><p className="mt-1 text-base leading-relaxed text-muted-foreground">Complete an estimate or receive a verified action completion to unlock a badge.</p></div></div>}<p className="mt-6 text-xs leading-5 text-muted-foreground">{summary?.disclaimer || "Points are available only when secure activity storage is available."}</p></article>
          </section>
        ) : (
          <section id="progress-contributors-panel" role="tabpanel" className="mt-12">
            <div className="cs-reference-contributors-heading"><span className="cs-progress-contributors-icon"><Globe2 size={28} /></span><div><h2 className="text-2xl font-bold tracking-tight text-foreground">Contributors Around the World</h2><p className="mt-3 text-base leading-7 text-muted-foreground">View consented aggregate participation across {countryCount} countr{countryCount === 1 ? "y" : "ies"}. Individual histories and member names are never shown.</p></div></div>
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
              <article className="cs-contributor-map-card cs-card p-6 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="cs-data-label">Aggregate contributor map</p><h3 className="mt-2 text-xl font-bold tracking-[-0.04em] text-foreground">Country coverage</h3></div><ShieldCheck className="text-primary" size={21} aria-hidden="true" /></div><ContributorAggregateMap countryRows={countryRows} onSelectCountry={setCountry} /><div className="cs-map-legend mt-5"><span>Fewer</span><i /><i /><i /><i /><i /><span>More</span></div><p className="mt-5 text-xs leading-5 text-muted-foreground">Zoom and pan to examine country-level aggregate coverage. The consent-filtered view refreshes while open, so new country coverage can appear without revealing an individual location or private activity history.</p></article>
              <aside className="cs-card p-6 sm:p-7"><div className="flex items-center justify-between gap-3"><div><p className="cs-data-label">Contributors</p><h3 className="mt-2 text-xl font-bold tracking-[-0.04em] text-foreground">Aggregate cohort</h3></div><span className="text-sm text-muted-foreground">{consentedParticipantCount} shown</span></div>{countryRows.length ? <div className="mt-6 space-y-3">{countryRows.map((row: ContributorRow) => <button type="button" key={row.country} onClick={() => setCountry(row.country)} className="cs-contributor-row"><UsersRound size={17} /><span>{row.country}</span><b>{row.value}</b></button>)}</div> : <div className="cs-contributors-empty"><Globe2 size={35} /><p className="mt-4 font-bold text-foreground">No contributors found for this selection.</p><p className="mt-2 text-base leading-relaxed text-muted-foreground">Coverage appears only when members consent to aggregate sharing.</p></div>}<div className="mt-7 border-t border-border pt-5 dark:border-white/10"><Link href="/quests" className="cs-action inline-flex items-center text-sm font-bold text-primary hover:underline">View privacy-aware leaderboard →</Link></div></aside>
            </div>
            <p className="mt-6 text-xs leading-5 text-muted-foreground">{contributors.data?.disclaimer || "Aggregate records are loading."}</p>
          </section>
        )}
      </main>
      <BackToTop />
